	...
	while ((c = fgetc(f)) != EOF)
	{
		if (status == 0)	//size_layer
		{
			...
			else
			{
				...
				printf("%d", size_layer);
				nnet->sizes_begin = realloc(nnet->sizes_begin, sizeof(size_t) * (num_layer+1));
				nnet->layers_begin = realloc(nnet->layers_begin, sizeof(struct Sig_Neuron) * sum_sizes); 
			}
		}
		...
		else
		{
			...
			else
			{
				...
				(nnet->layers_begin + cur-1)->weights_begin[idx] = strtod(read, NULL);
				idx++;
				if(c =='\n')
				{
					if(count == size_layer)
...
void savennet(struct Neural_Net* nnet, char* path)
{
	...
	int k = 0;
	   ...
	   //fprintf(f, "\n");
	      ...
	      fprintf(f, "%f", ((y+i)->weights_begin[*(nnet->sizes_begin + k - 1)-1]));
            }
            fprintf(f, "\n");
         }
