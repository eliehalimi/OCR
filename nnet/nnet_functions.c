/*
** nnet_functions.c : This is where the functions used by the neural networks
** will be stored
*/
...
void layer_init(struct Sig_Neuron* prev_layer_begin, struct Sig_Neuron*
	       	layer_begin, struct Sig_Neuron* layer_end)
{
	for(int i = 0; i < layer_end - layer_begin; i++)
	{
		...
		One can remark that prev_layer_end = layer_begin-1 which gives
			...
			(double*) malloc((layer_begin - prev_layer_begin)
				       	* sizeof(double));
	 	(layer_begin+i)->weights_end =
		(layer_begin+i)->weights_begin +
	       		(layer_begin - 1 - prev_layer_begin);
...
void net_init(struct Neural_Net *nnet)
{
	...
	{
		(nnet->layers_begin + i)->weights_begin =
...
void success_and_errors(struct Neural_Net* nnet, double* expect_begin) 
{
  	...
  	for(size_t i = 0; i < *(nnet->sizes_end - 1); i++) 
	{
		...
		if((actual+i)->output != 0 ||(actual+i)->output != 1 ||
			       	*(expect_begin + i) != 0)
	       	{
      			cost += ( -(actual+i)->error * log((actual+i)->output)-
					(1 - (actual+i)->error) *
				       	log(1 - (actual+i)->output));
		}
	}
